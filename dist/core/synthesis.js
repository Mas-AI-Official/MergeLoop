...
function extractStructuredSummary(result) {
...
export function buildSynthesisInputs(results) {
    return results.map(extractStructuredSummary);
}
export function detectDisagreements(synthesisInputs, results) {
    ...
    for (let index = 0; index < successful.length; index += 1) {
        for (let inner = index + 1; inner < successful.length; inner += 1) {
            ...
            if (similarity(left.summary, right.summary) < 0.35) {
                disagreements.add(`${left.worker_name} and ${right.worker_name} produced materially different summaries or priorities.`);
            }
...
export function recommendNextChecks(synthesisInputs, results) {
    ...
    for (const result of results) {
        ...
        if (result.status === "error") {
            nextChecks.add(`Verify ${result.worker_name} is installed on PATH and authenticated in the official CLI.`);
        }
    }
...
export function formatCouncilRunMarkdown(result) {
    ...
    for (const worker of result.results) {
        lines.push(`### ${worker.worker_name}`, `- Status: ${worker.status}`, `- Command: \`${worker.command}\``, worker.stderr.trim() ? `- stderr: ${worker.stderr.trim()}` : `- stderr: none`, `- stdout:`, "```text", (worker.stdout || "(empty)").trim(), "```");
    }

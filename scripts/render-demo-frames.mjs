...
function frameTemplate(title, body, step) {
...
async function main() {
  ...
  const frames = [
    frameTemplate("CouncilKit Demo", "Start from one prompt inside your host/editor.", 1),
    frameTemplate("Parallel Workers", "Codex, Gemini, and local workers run in parallel.", 2),
    frameTemplate("Unified Result", "Get one synthesis-ready answer with explicit checks.", 3)
  ];
...
<html lang="en">
  ...
  <body>
    <main>
      ...
      <p>Regenerate these frames with <code>npm run demo:render</code>.</p>

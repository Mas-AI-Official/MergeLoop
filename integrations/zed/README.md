# Zed Integration

Support level: `documented/manual`.

Template file: [`settings.json`](./settings.json)

## What Is Tested In This Repo

- Template shape and MergeLoop MCP registration pattern.
- Zed runtime/plugin behavior is user-environment specific and not CI-verified here.

## Known-Good Local Test Steps

1. Build MergeLoop: `npm ci && npm run build`
2. Add `mergeloop-hub` MCP entry in Zed config using absolute path.
3. Reload Zed.
4. Run an explicit `mergeloop_run` prompt and inspect returned structured fields.
